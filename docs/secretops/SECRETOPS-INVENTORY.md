...
| Cloudflare DNS API 토큰 | `clusters/dev/secretops/externalsecret-cloudflare-dns.yaml`의 `API_TOKEN` → `api-token` | `external-dns` | ExternalDNS 컨트롤러 (`CF_API_TOKEN`) | AWS Secrets Manager → ExternalSecret | 민감정보 | `cloudflare-api-token` / `api-token` | `gitops-argocd`, `aws-eks-terraform-lab` (`/aws-eks-terraform-lab/dev/dns/cloudflare`) | ASM 컨테이너와 값을 먼저 준비한 뒤 ExternalSecret, external-dns Application 순으로 적용. 토큰은 `Zone:DNS:Edit` + `Zone:Zone:Read`로 `lhrm-lab.com` 단일 zone 한정. 회전 시 `rollout restart` 필요. Rollback: Git revert 후 남은 Secret 수동 삭제 |
